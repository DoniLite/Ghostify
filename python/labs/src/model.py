...
class HybridBERTGPT2Model(nn.Module):
    ...
    def encode_input(self, text, return_tensor=True):
        ...
        if not text:
            return torch.zeros(1, self.bert_encoder.config.hidden_size) if return_tensor else None
    ...
    def generate_text(self, prompt, max_length=100):
        ...
        generation_output = self.gpt2_generator.generate(
            ...
            attention_mask=inputs.get('attention_mask'),  # Passer attention_mask séparément
        ...
        try:
            return self.gpt2_tokenizer.decode(generation_output[0], skip_special_tokens=True)
