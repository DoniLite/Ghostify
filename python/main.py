...
from python.utils.security import InternalJWTBearer, JWTBearer, TokenModel, create_access_token, decode_token
...
@app.post('/parser', dependencies=[Depends(token_bearer)])
async def parser(data: Annotated[ApiParserModel, Body]):
...
@app.post('/token', dependencies=[Depends(internal_bearer)])
async def get_token(token_payload: Annotated[TokenModel, Body]) -> Token:
