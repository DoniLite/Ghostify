...
const env = {
  WEBSOCKET_BASE_URL: process.env.WEBSOCKET_BASE_URL || 'ws://localhost:8787/ws/document/',
...
async function runBuild() {
  if (!flags.preview) {
    ...
    const entryPoints = glob
      .sync(['./src/frontend/**/*.{ts,js,jsx,tsx}', './src/client/**/*.{ts,js,jsx,tsx}'])
    ...
    // Copy static assets
    await fs.cp(path.join(root, 'static/css'), path.join(root, 'dist/css'), { recursive: true })
    copyFileSync(path.join(root, 'static/ghostify.svg'), path.join(root, 'dist/ghostify.svg'))
    copyFileSync(path.join(root, 'preview.html'), path.join(root, 'dist/index.html'))
  }
