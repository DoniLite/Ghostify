...
export type DeepKeyOf<T> = {
  [K in keyof T]: T[K] extends object ? K | Join<K, DeepKeyOf<T[K]>>
    : K;
}[keyof T];

export type StringToPath<S extends string> = S extends
  `${infer Head}.${infer Tail}` ? [Head, ...StringToPath<Tail>]
  : [S];

export type PathValue<T, Path extends readonly string[]> = Path extends
  [infer Head, ...infer Tail]
  ? Head extends keyof T ? Tail extends string[] ? PathValue<T[Head], Tail>
    : T[Head]
  : never
  : T;
