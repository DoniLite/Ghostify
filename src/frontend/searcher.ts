...
const componentConstructor = (data: Record<string, unknown>) => {
  ...
  if (typeof data['title'] === 'string') {
    return `<div class="w-full flex gap-x-2 items-center">
          ...
          <a href="${
      typeof data.url !== 'string'
        ? typeof data.uid !== 'string' ? data.link : `page/${data.uid}`
        : data.url
...
const sendKeysToServer = async (e: Event) => {
  ...
  if (!res.ok) {
    ...
    const errorComponent =
      `<div class=" text-red-600 font-bold text-center">Une erreur s'est produite</div>`;
