...
const requestFile = async (e) => {
  ...
  inputFile.onchange = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        const component = `
                <div id="imageView" class=" lg:w-5/6 w-11/12 mx-auto p-1 lg:max-h-48 max-h-32 justify-center items-center bg-gray-950 rounded-lg py-6 my-5">
                ...
                `;
        posterForm.insertAdjacentHTML("beforeend", component);
        const newImput = `<input type="file" name="file" class="hidden" id="file_${index + 1}" />`;
...
const addListItem = (e) => {
  ...
  const icr = Number(lastParent.querySelector('.lst-component').lastElementChild.getAttribute('id'))+ 1;
