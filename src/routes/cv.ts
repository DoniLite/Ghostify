...
cvApp.get('/load/:cv', async (c) => {
  ...
  try {
    ...
    const cvTheme = resumeClasses[`v${cvType as 1 | 2 | 3}`][`mode${cvMode as 1 | 2 | 3 | 4 | 5}`]
    ...
    cvObject.birthday =
      api && api === 'true' ? format(cvDate, 'yyyy-MM-dd') : format(cvDate, 'dd /MM/yyyy')
...
cvApp.get('/status', async (c) => {
  ...
  if (session.get('JobsIDs')) {
    ...
    if (status === 'completed') {
      ...
      return c.json({ success: true, img: cvData?.screenshot, doc: cvData?.pdf })
    }
    if (status === 'failed') {
      await cvJob?.retry()
      return c.json({
    ...
    if (status === 'active' || status === 'waiting') {
      return c.json({
        success: false,
        message: 'your CV is being processed, you will get notified once it is ready'
      })
    }
    return c.json({
      success: false,
